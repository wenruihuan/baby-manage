...
import {
    ...
    setPublish,
    getDiscountSoldList,
    getDiscountSoldDetail,
    getDiscountHisList,
    getDiscountHisDetail
    ...
    methods: {
        ...
        async getDefaultImg () {
            try {
                ...
                console.log(`src/components/page/goodsmanage/card-item/component/discount-view.vue error: ${e}`);
            }
        },
        /* 去编辑页 */
        gotoEdit () {
            const id = this.$route.query.id;
            this.$router.push(`/discount-card?id=${ id }`);
        },
        /* 上下架 */
        ...
        async hanldeCardView1 (row, index) {
            ...
            try {
                const data = await getDiscountSoldDetail({ member_card_id: row.member_card_id });
                if (data.code === ERR_OK) {
                ...
                console.log(`goodsmanage/card-item/component/discount-view.vue hanldeCardView1 error: ${ e }`);
            }
        },
        /* 查看历史卡详情 */
        async hanldeCardView2 (row, index) {
            this.dialogVisible = true;
            try {
                const data = await getDiscountHisDetail({ card_discount_id: row.card_discount_id });
